# Written by Lex Whalen https://github.com/lxaw/PoseAngleEstimation
...
class PoseEstimator():
    ...
    # Written by Sam Good
    def extract_knee_angles_from_video(video_path):
        ...
        while cap.isOpened():
            ...
            # Process the frame to get angles
            processed_frame = pose_estimator.get_pose_key_angles(frame)

            # Extract left and right knee angles
            left_knee_angle = pose_estimator.KEY_ANGLES["LLeg"][-1] if pose_estimator.KEY_ANGLES["LLeg"] else None
            right_knee_angle = pose_estimator.KEY_ANGLES["RLeg"][-1] if pose_estimator.KEY_ANGLES["RLeg"] else None
        ...
        avg_left_knee_angle = np.mean(left_knee_angles) if left_knee_angles else None
        avg_right_knee_angle = np.mean(right_knee_angles) if right_knee_angles else None

        return avg_left_knee_angle, avg_right_knee_angle
